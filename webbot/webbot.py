...
class Browser:
    ...
    **Constructor**
    ...
    def __init__(self, showWindow=True, proxy=None, downloadPath:str=None, driverPath:str=None, webdriverInstance:str='Chrome', useSeleniumGrid=False, experimentalOptions=[{"name":"", "value": ""}], arguments=["--disable-dev-shm-usage","--no-sandbox"]):
        options = getattr(webdriver, webdriverInstance + 'Options', lambda: None)()

        for argument in arguments:
            options.add_argument(argument)

        for experimentalOption in experimentalOptions:
            options.add_experimental_option(experimentalOption["name"], experimentalOption["value"])
        ...
        [setattr(self, function, getattr(self.driver, function)) for function in
         ['add_cookie', 'delete_all_cookies', 'delete_cookie', 'execute_script', 'execute_async_script',
          'fullscreen_window', 'get_cookie', 'get_cookies', 'get_log', 'get_screenshot_as_base64', 
          'get_screenshot_as_file', 'get_screenshot_as_png', 'get_window_position', 'get_window_rect', 
          'get_window_size', 'maximize_window', 'minimize_window', 'implicitly_wait', 'quit', 'refresh', 
          'save_screenshot', 'set_page_load_timeout', 'set_script_timeout', 'set_window_position', 
          'set_window_rect', 'start_client', 'start_session', 'stop_client']]
